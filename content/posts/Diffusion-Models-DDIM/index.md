...
[In Part 1]({{< relref "../Intro-Diffusion-Models-part1/index.md" >}}), we introduced *Denoising Diffusion Probabilistic Models (DDPMs)*[^Ho2020] and saw how they gradually turn noise into meaningful data. 
A key drawback of DDPMs is the slow sampling speed -- often requiring hundreds or thousands of reverse-diffusion iterations to generate high-quality samples.

In this post, we explore *Denoising Diffusion Implicit Models (DDIMs)*[^JSong2020], a non-Markovian, deterministic variant of DDPMs that accelerates sampling without retraining. 
DDIMs follow the same generative principle as DDPMs but take larger, more structured steps, enabling high-quality results with far fewer iterations.
...
\begin{equation}
\mathbf{x}\_t = \sqrt{\bar{\alpha}\_t}\mathbf{x}\_0 + \sqrt{1 - \bar{\alpha}\_t}\boldsymbol{\epsilon} \quad \quad ; \text{where} \ \boldsymbol{\epsilon} \sim \mathcal{N}(0,\boldsymbol{I})
\end{equation}
...
This change does not carry any physical meaning but simplifies notation. Under this form, the cumulative product becomes:
...
These marginals are identical to those of DDPM, which means *a DDPM-trained model can be used directly for DDIM sampling*.
...
To eliminate the direct dependence on the random noise, we solve Eqn \eqref{eq:6} for $\boldsymbol{\epsilon}$:
...
This formulation removes the need for explicit sampling from a Gaussian at every timestep.
Instead, $\mathbf{x}_{t-1}$ is now a deterministic function of $\mathbf{x}_t$ and an estimate of $\mathbf{x}_0$.
...
<div style="padding: 0px 12px 8px 12px;text-align: justify;font-style: italic;">
With no new random noise to add or undo, the sampler no longer needs many small corrective steps. 
Each update becomes stable and predictable, which allows DDIM to skip large portions of the trajectory and generate high-quality samples in far fewer iterations.
</div>
...
Unlike the DDPM forward process, the DDIM forward process in Eqn \eqref{eq:ddim-forward} allows each state $\mathbf{x}\_t$ to depend on both $\mathbf{x}\_{t-1}$ and the clean signal $\mathbf{x}\_0$. This is a key distinction from DDPM that makes the process *non-Markovian and no longer "memoryless"*.
...
The most important argument in DDIM is that: for all time $t$, we want the marginal distribution $q_{\sigma}(\mathbf{x}\_{t} \vert \mathbf{x}\_0)$ to have the same form: 
...
><mark class="gray">*Check out this tutorial[^Chan2024] to see the detailed derivation of this transition distribution.*</mark>
